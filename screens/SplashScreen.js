...
function SplashScreen({ navigation }) {
    ...
    const requestOtp = async () => {
        try {
            const {
                data: { message, Error },
            } = await axios.post("https://deal-stryker-backend.herokuapp.com/v1/users/verify-email", { email: email });
    ...
    async function SignUpUser(val) {
        ...
        try {
            let res = await axios.post("https://app.dealstryker.com/api/users/registerCustomer", bodySign);
            ...
            //         const {
            //             data: { message, Error },
            //         } = await axios.post("https://deal-stryker-backend.herokuapp.com/api/v1/users/signup", body);
            //         if (!Error) {
            //             setVisible(true);
            //             setDisplayMsg(message);
            //             setForSign(true);
    ...
    useEffect(async () => {
        ...
        console.log(AsyncStorage.getItem("token"), "bus");
    }, []);
    return (
